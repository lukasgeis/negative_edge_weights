...
do
    ...
    # BellmanFord
    ./target/release/random_negative_weights -w=-100 -W 100 -r $ROUNDS -t f64 -a bf gnp -n $NODES -d $DEGREE >> "$OUTPUT/gnp.out"
    ./target/release/random_negative_weights -w=-100 -W 100 -r $ROUNDS -t f64 -a bf rhg -n $NODES -d $DEGREE >> "$OUTPUT/rhg.out"
    ./target/release/random_negative_weights -w=-100 -W 100 -r $ROUNDS -t f64 -a bf dsf -n $NODES -d $DEGREE >> "$OUTPUT/dsf.out"
done
