...
for _, row in log.iterrows():
    ...
    data["rate"].append(
        (row["num_ins_dk_acc"] + row["num_ins_dk_rej"]) / row["num_acc_rounds"]
    )
    data["algo"].append("DK")
    ...
    data["round"].append(row["round"])
    data["rate"].append(
        (row["num_ins_bd_acc"] + row["num_ins_bd_rej"]) / row["num_acc_rounds"]
    )
    data["algo"].append("BD")
...
if args.degree is not None:
    ...
    legend = plt.legend(handles, labels, ncols=8, fontsize=13, bbox_to_anchor=(1.15, -0.25))

    fig.text(0.515,-0.08, r'\textsc{Rounds}', ha="center", fontsize=20)
    fig.text(0.065, 0.50, r'\textsc{Insertions per}' "\n" r'\textsc{Weight Update}', va="center", rotation="vertical", fontsize=17)
    ...
    legend = plt.legend(handles, labels, ncols=8, fontsize=13, bbox_to_anchor=(1.15, -0.3))

    fig.text(0.515, 0.06, r'\textsc{Rounds}', ha="center", fontsize=20)
    fig.text(0.053, 0.50, r'\textsc{Insertions per}' "\n" r'\textsc{Weight Update}', va="center", rotation="vertical", fontsize=17)
